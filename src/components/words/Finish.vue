    ...
    <div class="finish">
      <div class="finish__content">
        <div class="finish__header">
          <h1>
            Вы написали верно {{ complete == null ? 0 : complete.length }}
            {{ wordsNum }} из {{ len }} {{ wordsNum }}
          </h1>
          <span
            v-if="mistakes?.length > 0 && mistakes != null"
            class="finish__error-title"
          >
            <p @click="isOpen = !isOpen">
              Было допущено {{ mistakes.length }} {{ errorsNum }}
            </p>
            <img
              v-show="!isOpen"
              @click="isOpen = true"
          ...
          <div v-if="isOpen" class="finish__error">
            <div
              class="finish__error-list"
              v-for="(err, index) of mistakes"
              :key="index"
            >
              <p>
                ...
                -
                <strike
                  class="finish__error-russian"
                  v-if="err.translation != null"
                >
                  {{ err.translation }}</strike
                >
              </p>
            </div>
          </div>
...
export default {
  ...
  computed: {
    errorsNum() {
      ...
      else if (this.mistakes.length > 1 && this.mistakes.length < 5)
        return "ошибки";
      else return "ошибок";
    },
    wordsNum() {
  ...
  methods: {
    returnStart() {
      ...
      this.$router.push(
        `/words/${this.$route.params.userid}/0/?type=englishV&task=1`
      );
    }
  }
};
