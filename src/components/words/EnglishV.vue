    ...
    <div class="english-form">
      <div class="nglish-form__content">
        <div class="english-form__header">
          ...
          <span v-if="wordData.currentTime != null" class="english-form-time"
            >({{ wordData.currentTime }})</span
          >
        </div>
        ...
        <div class="english-form__result">
          ...
          <p v-if="isError.value" class="english-form__danger">
            <span v-for="(err, index) of errorsList.value" :key="index">
              ...
              >
                Ответ неверный! Возможно перевод этого слова раннее был введен
                неккоректно.
              </p>
            </span>
          </p>
        ...
        <div class="english-form__footer">
          <button
            @click="$emit('previousTask')"
            class="profile__run"
            v-if="parseInt(taskNum) > 1"
          >
            Вернуться
          </button>
          <button
    ...
    const addSuccessWord = () => {
      if (isDone.value) {
        ...
        if (arrayWords?.length > 0 && arrayWords != null) {
          arrayWords.map(item => {
            if (item.translation != newSuccessWord.translation)
              newArrayWords.push(item);
          });
        }
